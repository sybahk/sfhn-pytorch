...
class Model(nn.Module):
    ...
    def forward_patch(self, x):
        ...
        if (w_size * h_size) < threshold:  # smaller than the threshold
            ...
            sr_list = [self.forward_patch(patch) for patch in lr_list]
    ...
    def forward_overlap(self, x):
        ...
        for i in range(num_sample):
            ...
            x_in = x[
                :, :, pos[0] : pos[0] + patch_size[0], pos[1] : pos[1] + patch_size[1]
            ]
            x_out = self.model(x_in).cpu()
            pos = [x * self.scale for x in pos]
            output[:, :, pos[0] : pos[0] + out_sz[0], pos[1] : pos[1] + out_sz[1]] = (
                x_out * ww
            )
