...
class checkpoint:
    ...
    def save(
        ...
        self.save_model(
            self.get_path("model"), trainer, iteration, is_best, is_swa, iter_suffix
        )
    ...
    def load_model(
        ...
        if isinstance(state["state_dict"], tuple):
            ...
            if ignore is not None:
                pretrained_dict = {
                    k: v for k, v in pretrained_dict.items() if ignore not in k
                }
...
def calc_psnr_torch(
    sr, hr, scale, rgb_range: float = 255.0, use_gray_coeffs: bool = True
):
