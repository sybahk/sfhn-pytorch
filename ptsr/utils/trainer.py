...
class Trainer:
    def __init__(self, cfg, rank, loader, model, loss, device, ckp=None):
        ...
        if not cfg.SOLVER.TEST_ONLY:
            ...
            self.scaler = (
                GradScaler(backoff_factor=0.91, growth_factor=1.1, growth_interval=5000)
                if self.mixed_fp
                else None
            )
        ...
        if self.cfg.AUGMENT.MIXUP.ENABLED:
            self.mixuper = Mixup(
                bs=self.cfg.SOLVER.SAMPLES_PER_BATCH, beta=self.cfg.AUGMENT.MIXUP.BETA
            )
            ...
            if iteration >= 0:
                ...
                if not self.cfg.SOLVER.TEST_ONLY:
                    ...
                    if best[0][idx_data, idx_scale] > self.best_val_score:
                        ...
                        is_best = True
                    self.ckp.save(
                        self, iteration, is_best, self.iter_start, is_swa=is_swa
                    )
    ...
    def maybe_save_swa_model(self):
        ...
        self.model.module.model.load_state_dict(
            self.swa_model.module.module.model.state_dict()
        )
        self.test(-1)
