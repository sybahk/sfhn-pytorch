...
from ptsr.model import get_num_params, FreqL1Loss
...
def get_args():
    ...
    parser.add_argument(
        "--config-base", type=str, help="base configuration file (yaml)", default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--local_world_size", type=int, default=1, help="number of GPUs each process."
    )
    parser.add_argument(
        "--local_rank",
        type=int,
...
def main():
    ...
    if args.distributed:  # parameters to initialize the process group
        ...
        manual_seed = args.local_rank if args.manual_seed is None else args.manual_seed
    else:
    ...
    checkpoint.load_model(
        ...
        trainer=t,
        device=device,
        restart=cfg.SOLVER.ITERATION_RESTART,
        test_mode=cfg.SOLVER.TEST_ONLY,
        strict=cfg.MODEL.CKP_STRICT,
        ignore=cfg.MODEL.CKP_IGNORE,
    )
...
def build_model_loss(cfg, rank, checkpoint, device):
    ...
    if not cfg.SOLVER.TEST_ONLY:
        if not cfg.SOLVER.LOSS.FREQ:
            _loss = nn.L1Loss().to(device)
        else:
            _loss = FreqL1Loss().to(device)
    return _model, _loss
